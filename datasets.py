...
class EmbeddingLoader():
    def __init__(self, path, size, maxlen, verbose=True):
        self.emb_path = path
        self.emb_size = size
        self.ind = maxlen
...
class DataLoader():
    def __init__(self, path, embedding, batch_size=64, training=True, lemmatize=False, verbose=True):
        self.path = path
    ...
    def transform(self):
        ...
        def get_indices(sentence):
            ...
            for word in sentence.split(' '):
                ind_ = self.emb.word_to_ix.get(word)
                if ind_ is None:
                    continue
                else:
                    res.append(ind_)
    ...
    def batch_generator(self):
        for i in range(0, len(self.text), self.batch_size):
            ...
            if self.training is True:
                yield (torch.LongTensor(list(map(lambda x:x + [self.emb.ind] * (max_len - len(x)), batch))), 
