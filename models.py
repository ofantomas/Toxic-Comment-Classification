...
class nnPredictor():
    def __init__(self, input_size, hidden_size, num_layers, num_classes, embeddings, batch_size=256, dropout=0.5, 
    ...
    def train(self, data_train, data_test=None, num_epochs=3, scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau):
        ...
        for epoch in range(num_epochs):
            with tqdm.tqdm(enumerate(data_train.batch_generator()), total=int(len(data_train) / self.batch_size)) as iterator:
    ...
    def test(self, data_test):
        ...
        with torch.no_grad():
            with tqdm.tqdm(enumerate(data_test.batch_generator()), total=int(len(data_test) / self.batch_size)) as iterator:
                for batch_num, batch in iterator:
                    text, labels = batch
                    labels = labels.to(self.device)
    ...
    def predict(self, dataloader):
        ...
        with torch.no_grad():
            with tqdm.tqdm(enumerate(dataloader.batch_generator()), total=int(len(dataloader) / self.batch_size)) as iterator:
                for batch_num, batch in iterator:
        ...
        res = res.reshape(-1, self.model.num_classes)
